...
def cek_stok():
    try:
        ...
        if res.status_code != 200:
            ...
            print(res.text)
            return
        # Simpan halaman untuk debug
        with open("debug_response.html", "w", encoding="utf-8") as f:
            f.write(res.text)
        print("[DEBUG] Halaman disimpan ke debug_response.html")
    except Exception as e:
    ...
    if not produk:
        print("❌ Tidak ada produk ditemukan. Cek struktur HTML atau cookie!")
        return
    ...
    for p in produk:
        print("\n===== DEBUG PRODUK HTML =====")
        print(p.prettify())  # Debug isi tiap produk

        # Ambil nama produk
        nama = p.find("h5").text.strip() if p.find("h5") else "Tanpa Nama"
        
        # Cek apakah ada tombol "Redeem Now"
        tombol = p.find("button", string=lambda t: "Redeem Now" in t if t else False)

        if tombol:
            # Jika tombol tersedia, kirim notifikasi
...
while True:
    print("=" * 20)
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Mengecek produk kategori 4...")
    cek_stok()
    time.sleep(30)
